...
- An approved `buy-sol` or `sell-sol` intent can request a real Raydium Devnet SOL/devUSDC quote with a fixed low-value canary amount. For an allowed `sell-sol` quote, the runtime can also ask Raydium for the unsigned V0 transaction, decode it locally, allowlist its signer, programs, lookup table, route pools, and exact input/minimum output, then simulate it without signature verification. Transaction material stays encrypted and the public receipt exposes only hashes and simulation evidence. `buy-sol` remains quote-only; neither path can sign or broadcast.
