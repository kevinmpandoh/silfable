...
- An approved `buy-sol` or `sell-sol` intent can request a real Raydium Devnet SOL/devUSDC quote with a fixed low-value canary amount. For an allowed `sell-sol` quote, the runtime can ask Raydium for the unsigned V0 transaction, decode it locally, allowlist its signer, programs, lookup table, route pools, exact input/minimum output, and the wallet's deterministic devUSDC associated token account. It snapshots the wallet and output account, simulates without signature verification, then requires the devUSDC increase to meet the quoted minimum and the SOL decrease to stay within input, fee, and bounded account-rent costs. Transaction material stays encrypted and the public receipt exposes only hashes and balance-delta evidence. `buy-sol` remains quote-only; neither path can sign or broadcast.
- A successful Raydium `sell-sol` proof no older than 15 seconds can receive a separate encrypted one-shot signing authority after three explicit acknowledgements. It is bound to the exact build, message hash, proposal digest, output ATA, and verified balance deltas; expires within 15 seconds; and is revoked on lock, suspend, quit, restart, approval change, session halt, or explicit action. This release deliberately has no consumer or signing bridge for that authority, so it still cannot load a key, sign, or broadcast the economic transaction.
