...
The economic Devnet bridge starts with an approved agent direction mapped to `sell-sol` SOL→devUSDC or `buy-sol` devUSDC→SOL at fixed canary amounts through Raydium's fixed Devnet Transaction API. Renderer input cannot select mints, amount, pool, route, slippage, endpoint, accounts, or transaction bytes. Main checks the quote chain, output threshold, pool IDs, session impact cap, and approval after latency, then encrypts the journal. For `sell-sol` only, a separate simulation-only service sends the exact stored Raydium quote to the fixed build endpoint, requires one unsigned V0 transaction and the dedicated wallet as its only signer, and allowlists the Raydium Devnet router, Compute Budget program, fixed lookup table, quoted mints, route pools, and deterministic wallet devUSDC ATA. It snapshots the system-owned wallet and existing output ATA before simulation, requests their post-simulation states from RPC, decodes SPL Token state with the official codec, and requires the output delta to meet the quote minimum while the wallet lamport delta stays within exact input, fee, and a bounded ATA-rent allowance. Slots cannot move backwards. It also hashes the message, enforces a fee cap, and repeats approval checks after RPC latency. Raw quote/build transactions stay encrypted. A separate economic signing-arm journal can authorize one future signature for at most 15 seconds, bound to the exact build, proposal, message, ATA, and deltas; database triggers and lifecycle hooks revoke it on approval/session or runtime safety changes. No component consumes that arm, and the service has no signer, transaction wire, or broadcaster dependency. `buy-sol` remains quote-only until its input devUSDC balance policy is implemented.
