...
The economic Devnet bridge starts with an approved agent direction mapped to `sell-sol` SOL→devUSDC or `buy-sol` devUSDC→SOL at fixed canary amounts through Raydium's fixed Devnet Transaction API. Renderer input cannot select mints, amount, pool, route, slippage, endpoint, or transaction bytes. Main checks the quote chain, output threshold, pool IDs, session impact cap, and approval after latency, then encrypts the journal. For `sell-sol` only, a separate simulation-only service sends the exact stored Raydium quote to the fixed build endpoint, requires one unsigned V0 transaction and the dedicated wallet as its only signer, and allowlists the Raydium Devnet router, Compute Budget program, fixed lookup table, quoted mints and route pools. It decodes the router instruction to prove exact input and minimum output, hashes the compiled message, simulates the unsigned wire, enforces a fee cap, and repeats approval checks after RPC latency. Raw quote/build transactions stay encrypted; no signer or broadcaster is reachable. `buy-sol` remains quote-only until token balance and associated-account policy are proven.
