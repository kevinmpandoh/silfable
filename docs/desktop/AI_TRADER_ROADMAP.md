...
Increment 7 adds transaction construction and exact simulation for an allowed `sell-sol` quote only. Main gives Raydium the exact encrypted quote response, accepts exactly one unsigned V0 transaction, and decodes it before simulation. The wallet must be the only signer; router, Compute Budget program, lookup table, quoted mints, and route pools are allowlisted; and the router instruction must contain the exact quoted input and minimum output. Fee, approval, expiry, and network checks fail closed, including after RPC latency. The unsigned wire remains encrypted and there is no signing or broadcast dependency. `buy-sol` stays quote-only until its devUSDC balance and associated-token-account behavior can be proven safely.

Increment 8 adds account ownership and balance-delta proof to that `sell-sol` simulation. Main derives the wallet's devUSDC ATA locally, requires it in the built message, snapshots the system wallet and any existing ATA, then requests the same accounts from the simulation result. Official SPL codecs must prove the output account's token program, mint, and wallet owner. The devUSDC delta must meet the quoted minimum; the wallet SOL delta must cover the exact input and fee without exceeding a bounded ATA-rent allowance; and the RPC context slot cannot move backwards. Foreign accounts, insufficient output, excessive SOL loss, malformed token state, or missing post-state deny the receipt. No signer or broadcaster consumes this evidence.

Increment 9 adds a separate revocable one-shot signing authority for an exact successful economic `sell-sol` proof. The build must be no older than 15 seconds, the authority expires within 15 seconds, and the encrypted journal binds the build, quote, evaluation, session, proposal digest, message hash, output ATA, and both verified balance deltas. Only one can be active; approval/session database triggers plus lock, suspend, quit, and restart revoke it. Its public contract hard-codes that no signing bridge is connected and no signing or broadcast was attempted. Exact pre-sign revalidation and atomic consumption remain unimplemented.
...
Phase 5 increments 1–9 are complete. The fixture pipeline reaches safe Devnet confirmation, while the separate economic path binds an approved AI direction to a capped Raydium Devnet SOL/devUSDC quote and, for `sell-sol`, a decoded, allowlisted, exact unsigned simulation with deterministic ATA and bounded balance-delta proof plus a 15-second revocable one-shot signing authority. Silfable still does not execute the economic swap: no component consumes that authority, loads a signer, signs, or broadcasts the Raydium transaction; `buy-sol` remains quote-only; and the mapping does not reproduce the full AI notional. The next increment is exact pre-sign revalidation and atomic arm consumption without broadcast. Every Mainnet signing/broadcast path remains disabled.
