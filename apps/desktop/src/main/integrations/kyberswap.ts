import type { EvmChainKey } from "@mirae/contracts";
...
export class KyberSwapQuoteService {
  ...
  async quote(input: {
    ...
    const response = await this.#fetcher(url, {
      headers: { Accept: "application/json", "x-client-id": "Mirae" },
  ...
  async build(input: {
    ...
    const response = await this.#fetcher(`${KYBER_BASE_URL}/${chain.kyberSlug}/api/v1/route/build`, {
      ...
      body: JSON.stringify({
        ...
        sender: input.sender,
        recipient,
        // Kyber V1 accepts percentage points, whereas Mirae stores bps.
