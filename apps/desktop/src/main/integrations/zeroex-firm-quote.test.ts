...
test("firm 0x quote rejects incomplete transaction data", async () => {
  await assert.rejects(() => getRobinhoodFirmQuote({ apiKey: "test-key", taker: a, sellToken: a, buyToken: "0x2222222222222222222222222222222222222222", sellAmount: "100", slippageBps: 100, fetcher: async () => new Response("{}", { status: 200 }) }), /incomplete/u);
});
