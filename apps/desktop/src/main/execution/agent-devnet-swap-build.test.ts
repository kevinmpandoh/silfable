...
import { blockhash, getCompiledTransactionMessageEncoder, getTransactionEncoder } from "@solana/kit";
import { AgentDevnetSwapQuoteViewSchema, type AgentIntentEvaluationView } from "@silfable/contracts";

import { AgentDevnetSwapBuildService, validateBuiltTransaction } from "./agent-devnet-swap-build";
...
test("compiled Raydium transaction binds signer, programs, lookup table, and exact amounts", () => {
  ...
  const valid = validateBuiltTransaction(wire, WALLET, evidence.view);
  assert.deepEqual(valid.programIds, [ROUTER, COMPUTE]); assert.equal(valid.messageHash.length, 64);
  assert.throws(() => validateBuiltTransaction(buildWire({ inputAmount: 2n }), WALLET, evidence.view), /amount-mismatch/u);
  assert.throws(() => validateBuiltTransaction(buildWire({ program: "11111111111111111111111111111111" }), WALLET, evidence.view), /program-denied/u);
  assert.throws(() => validateBuiltTransaction(buildWire({ lookup: "11111111111111111111111111111111" }), WALLET, evidence.view), /build-invalid/u);
});
...
function fixture() {
  ...
  const service = (options: { wire?: string; simulationError?: boolean; beforeSimulation?: () => void; isApproved?: () => boolean } = {}) =>
    new AgentDevnetSwapBuildService({ database: database as unknown as RuntimeDatabase, cipher,
      ...
      rpc: { async simulateTransaction() { options.beforeSimulation?.(); return { error: options.simulationError ?? false, unitsConsumed: 2000n, fee: 5000n }; } },
...
function buildWire(options: { inputAmount?: bigint; program?: string; lookup?: string } = {}) {
  ...
  const messageBytes = getCompiledTransactionMessageEncoder().encode({ version: 0, header: { numSignerAccounts: 1,
    ...
    "So11111111111111111111111111111111111111112", "4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU", POOL,
    options.program ?? ROUTER, COMPUTE], lifetimeToken: blockhash("11111111111111111111111111111111"), instructions: [
      { programAddressIndex: 4, accountIndices: [0, 1, 2, 3], data }, { programAddressIndex: 5, data: Uint8Array.of(2, 1) }],
