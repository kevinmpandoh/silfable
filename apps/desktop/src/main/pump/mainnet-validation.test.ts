...
import type { PumpTradeContractPreview } from "@mirae/contracts";
...
test("Minimum-value Mainnet Buy Validation Matrix: 1,000 lamport buy proposal", async () => {
  ...
  assert.equal(buildResult.codec, "mirae-pump-v2");
...
test("Full End-to-End Restricted Execution Pipeline Validation Matrix", async () => {
  const directory = await mkdtemp(join(tmpdir(), "mirae-mainnet-matrix-"));
