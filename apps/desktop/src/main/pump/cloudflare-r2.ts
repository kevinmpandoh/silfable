...
import { R2SettingsSchema, type PumpLaunchDraft, type PumpLaunchMetadataPackage, type R2Settings } from "@mirae/contracts";
...
export class CloudflareR2Service {
  ...
  async publishLaunchMetadata(draft: PumpLaunchDraft): Promise<PumpLaunchMetadataPackage> {
    ...
    const id = randomUUID();
    const key = `mirae/token-launch/${draft.id}/metadata-${id}.json`;
    const response = await this.#signedFetch({
      settings,
      credentials,
      method: "PUT",
