    ...
    if (url.endsWith("/api/auth/wallet/verify")) {
      return new Response(JSON.stringify({
        ...
        headers: {
          ...
          "set-cookie": "mirae_wallet_session=test-session; Path=/; HttpOnly; Secure",
        },
      });
    }
    assert.equal((init?.headers as Record<string, string>).cookie, "mirae_wallet_session=test-session");
