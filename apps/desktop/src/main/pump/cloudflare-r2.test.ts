...
const settings = { accountId: "a".repeat(32), bucket: "mirae-launches", publicBaseUrl: "https://assets.example.com" };
...
test("R2 metadata publication is an explicit object upload and never makes a blockchain request", async () => {
  ...
  assert.match(requests[0]?.url ?? "", /cloudflarestorage\.com\/mirae-launches\/mirae\/token-launch/u);
  assert.match(requests[0]?.body ?? "", /"image":"https:\/\/assets\.example\.com\/example\.png"/u);
  assert.match(published.uri, /^https:\/\/assets\.example\.com\/mirae\/token-launch\//u);
});
