...
test("start DCAing prompt phrasing binds directly to the active session", { concurrency: false }, async () => {
  const secrets = new MemorySecrets();
  secrets.values.set("openrouter-api-key", "sk-or-test");
  let createInput: Record<string, unknown> | null = null;
  let providerFetches = 0;
  ...
  const service = new AiService({
    ...
    automationManager: {
      createDca: (input: Record<string, unknown>) => {
        createInput = input;
        return { id: "strategy-aapl-dcaing", ...input };
      },
    } as never,
  });
  const result = await service.chat({
    prompt: "start DCAing $1 USDC into AAPL every 2 minutes",
    mode: "agent",
    walletAddress: "2r2pXUspsXamwzNWc8dQn52GK2BJJWmr63MPzDDxjTcg",
    walletScope: "solana",
    sessionId: "session-aapl-123",
