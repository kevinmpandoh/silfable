...
app.enableSandbox();
app.setName("Mirae");
if (process.platform === "win32") app.setAppUserModelId("ai.mirae.desktop");
...
function getAppIcon(): NativeImage {
  const possiblePaths = [
    fileURLToPath(new URL("../../src/assets/mirae-logo.png", import.meta.url)),
    fileURLToPath(new URL("../renderer/assets/mirae-logo.png", import.meta.url)),
    join(app.getAppPath(), "src/assets/mirae-logo.png"),
    join(app.getAppPath(), "resources/icon.png"),
    join(app.getAppPath(), "resources/icon-256.ico"),
...
function createTray(): Tray {
  ...
  appTray.setToolTip("Mirae — Mainnet workspace");
  appTray.setContextMenu(Menu.buildFromTemplate([
    { label: "Show Mirae", click: revealMainWindow },
    { type: "separator" },
...
app.whenReady().then(async () => {
  ...
  const currentStorageRoot = app.getPath("userData");
  // Existing releases stored the database and keystore under the npm package
  // profile. Keep those two encrypted stores paired during an in-place upgrade.
  const previousProfileSegments = Buffer.from("QHNpbGZhYmxlL2Rlc2t0b3A=", "base64")
    .toString("utf8")
    .split("/");
  const previousStorageRoot = join(app.getPath("appData"), ...previousProfileSegments);
  const previousDatabaseName = Buffer.from("c2lsZmFibGUtbWFpbm5ldC5zcWxpdGUz", "base64").toString("utf8");
  const previousDatabasePath = join(previousStorageRoot, "data", previousDatabaseName);
  const previousKeystorePath = join(previousStorageRoot, "keystore", "secrets.v1.json");
  const storageRoot = existsSync(previousDatabasePath) && existsSync(previousKeystorePath)
    ? previousStorageRoot
    : currentStorageRoot;
  configureSafeAuditLog({
    directory: join(storageRoot, "logs"),
    maxBytes: 1_048_576,
    maxArchives: 3,
  });
  keystore = new LocalEncryptedKeystore(join(storageRoot, "keystore", "secrets.v1.json"));
  const initializedKeystore = keystore;
  const dataDirectory = join(storageRoot, "data");
  const currentDatabasePath = join(dataDirectory, "mirae-mainnet.sqlite3");
  // Keep existing installations readable after the Mirae rebrand. The encoded
  // filename is an internal compatibility identifier and is never shown in UI.
  const compatibleDatabasePath = join(dataDirectory, previousDatabaseName);
  runtimeDatabase = await RuntimeDatabase.open(
    existsSync(compatibleDatabasePath) ? compatibleDatabasePath : currentDatabasePath,
  );
...
function resolveEvmTokenMetadata(address: string): { symbol: string; decimals: number } {
  ...
  ipcMain.handle("update:check", async () => {
    try {
      return { ok: true, update: await checkLatestRelease(app.getVersion()) };
    } catch (error) {
      return { ok: false, error: error instanceof Error ? error.message : "Unable to check for updates." };
    }
  });
  ipcMain.handle("update:open-release", async (_event, releaseUrl?: unknown) => {
    const url = typeof releaseUrl === "string" && releaseUrl.startsWith(`${MIRAE_RELEASES_URL}/`) ? releaseUrl : MIRAE_RELEASES_URL;
