...
function registerIpc(
  ...
  ipcMain.handle(IPC_CHANNELS.walletUnlock, async (event, untrustedRequest: unknown) => {
    ...
    await agentDevnetBroadcast.reconcilePending();
  ...
  ipcMain.handle(IPC_CHANNELS.agentArmDevnetSigning, async (event, untrustedRequest: unknown) => {
    assertTrustedSender(event);
    const request = AgentArmDevnetSigningRequestSchema.parse(untrustedRequest);
    return AgentDevnetSigningArmMutationResponseSchema.parse({
      schemaVersion: 1,
      requestId: request.requestId,
      arm: await agentDevnetSigningArms.arm({
        simulationId: request.simulationId,
        expectedProposalDigest: request.expectedProposalDigest,
        expectedMessageHash: request.expectedMessageHash,
      }),
    });
  });

  ipcMain.handle(IPC_CHANNELS.agentRevokeDevnetSigningArm, async (event, untrustedRequest: unknown) => {
    assertTrustedSender(event);
    const request = AgentRevokeDevnetSigningArmRequestSchema.parse(untrustedRequest);
    return AgentDevnetSigningArmMutationResponseSchema.parse({
      schemaVersion: 1,
      requestId: request.requestId,
      arm: await agentDevnetSigningArms.revoke(request.signingArmId),
    });
  });

  ipcMain.handle(IPC_CHANNELS.agentListDevnetSigningArms, async (event) => {
    assertTrustedSender(event);
    return AgentDevnetSigningArmListResponseSchema.parse({
      schemaVersion: 1,
      arms: await agentDevnetSigningArms.list(),
    });
  });
  ...
  ipcMain.handle(IPC_CHANNELS.agentBroadcastDevnetExecution, async (event, untrustedRequest: unknown) => {
    assertTrustedSender(event);
    const request = AgentBroadcastDevnetExecutionRequestSchema.parse(untrustedRequest);
    return AgentBroadcastDevnetExecutionResponseSchema.parse({ schemaVersion: 1, requestId: request.requestId,
      execution: await agentDevnetBroadcast.broadcast(request.signedExecutionId, request.expectedMessageHash, request.expectedSignatureHash) });
  });
  ipcMain.handle(IPC_CHANNELS.agentListDevnetBroadcastExecutions, async (event) => {
    assertTrustedSender(event);
    return AgentDevnetBroadcastExecutionListResponseSchema.parse({ schemaVersion: 1, executions: await agentDevnetBroadcast.list() });
  });
  ipcMain.handle(IPC_CHANNELS.agentQuoteDevnetSwap, async (event, untrustedRequest: unknown) => {
    assertTrustedSender(event); const request = AgentQuoteDevnetSwapRequestSchema.parse(untrustedRequest);
    return AgentQuoteDevnetSwapResponseSchema.parse({ schemaVersion: 1, requestId: request.requestId,
      quote: await agentDevnetSwapQuotes.quote(request.evaluationId) });
  });
  ipcMain.handle(IPC_CHANNELS.agentListDevnetSwapQuotes, async (event) => {
    assertTrustedSender(event);
...
app.whenReady().then(async () => {
  ...
  const agentDevnetSigningArms = new AgentDevnetSigningArmService({
    database: runtimeDatabase,
    cipher: dataCipher,
    keystore,
    simulations: agentDevnetSimulations,
    agents: agentSessions,
    fixtures: fixtureReview,
  });
  ...
  const agentDevnetBroadcast = new AgentDevnetBroadcastService({
    database: runtimeDatabase, cipher: dataCipher, keystore, health: networkMonitor,
    fixtures: fixtureReview, agents: agentSessions, simulations: agentDevnetSimulations,
    signing: agentDevnetSigning, chain: new SolanaAgentDevnetBroadcastAdapter(devnetRpc),
  });
