...
export async function checkLatestRelease(currentVersion: string, request: typeof fetch = fetch): Promise<DesktopUpdateInfo> {
  const response = await request(RELEASES_API, {
    headers: { Accept: "application/vnd.github+json", "User-Agent": `Mirae-Desktop/${currentVersion}` },
  ...
  if (payload.draft === true || payload.prerelease === true || typeof payload.tag_name !== "string") throw new Error("No stable Mirae release is available.");
  ...
  const releaseUrl = typeof payload.html_url === "string" && payload.html_url.startsWith(`${RELEASES_URL}/`) ? payload.html_url : RELEASES_URL;
  return {
    currentVersion,
    latestVersion,
...
export const MIRAE_RELEASES_URL = RELEASES_URL;
