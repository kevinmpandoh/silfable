...
const api = {
  ...
  async executeFullAccessMission(request: import("@mirae/contracts").MissionFullAccessExecuteRequest) {
  ...
  async createFullAccessSolanaSwapJob(request: import("@mirae/contracts").FullAccessExecutionCreateSolanaSwapJobRequest) {
  ...
  async verifyFullAccessSessionEnrollment(request: import("@mirae/contracts").FullAccessSessionEnrollmentRequest) {
  ...
  async checkForUpdates() {
    return await ipcRenderer.invoke("update:check") as { ok: boolean; update?: { currentVersion: string; latestVersion: string; available: boolean; releaseUrl: string; publishedAt: string | null }; error?: string };
  },
  async openUpdateRelease(releaseUrl: string) {
    return await ipcRenderer.invoke("update:open-release", releaseUrl) as { ok: boolean };
  },
  ...
  async preparePerpOrder(request: any) {
    return await ipcRenderer.invoke(IPC_CHANNELS.perpsOrderPrepare, request) as { proposal: import("@mirae/contracts").PerpProposal };
  },
  async executePerpOrder(request: any) {
    return await ipcRenderer.invoke(IPC_CHANNELS.perpsOrderExecute, request) as { signature: string };
  },
  async getDriftMarkets() {
    return await ipcRenderer.invoke(IPC_CHANNELS.perpsMarketsGet) as { markets: any[] };
  },
  async getDriftAccount(walletAddress: string) {
    return await ipcRenderer.invoke(IPC_CHANNELS.perpsAccountGet, walletAddress) as { account: any };
  },
  async prepareDriftOrder(request: any) {
    return await ipcRenderer.invoke(IPC_CHANNELS.perpsOrderPrepare, request) as { proposal: any };
  },
};

contextBridge.exposeInMainWorld("mirae", Object.freeze(api));
