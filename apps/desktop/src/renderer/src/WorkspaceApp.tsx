...
function MainWorkspace({
 ...
 const [preparingBridgeIds, setPreparingBridgeIds] = useState<string[]>([]);
  ...
  const [showDriftPerpsPanel, setShowDriftPerpsPanel] = useState(false);

  async function handlePerpsPanelSubmit(request: {
    symbol: string;
    ...
    try {
      setShowDriftPerpsPanel(false);
      const res = await (window as any).mirae?.preparePerpOrder?.({
  ...
  function persistSession(session: SessionItem): Promise<unknown> {
    return window.mirae.upsertSession({
      schemaVersion: 1,
  ...
  async function prepareBridge(
    ...
    try {
      ...
      await window.mirae.prepareBridge({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: target.id,
  ...
  useEffect(() => {
   ...
   async function confirmDeleteSession(): Promise<void> {
    ...
    try {
      await window.mirae.deleteSession(sessionToDelete.id);
  ...
  async function requestSession(prompt = ""): Promise<void> {
    ...
    setModalOpen(true);
    try {
      const latestRuntime = await window.mirae.getRuntimeStatus();
      ...
      const [solanaResponse, evmResponse] = await Promise.all([
        window.mirae.listWallets(),
        window.mirae.getRobinhoodWallet().catch(() => null),
      ]);
  ...
  async function dispatchFullAccessEvmBridge(input: {
    ...
    try {
      ...
      for (let step = 0; step < 2; step += 1) {
        const result = await window.mirae.executeEvmBridge({
          schemaVersion: 1,
          requestId: crypto.randomUUID(),
          sessionId: input.sessionId,
  ...
  async function runSimulation(input: {
    ...
    try {
      const response = await window.mirae.simulateMission({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
  ...
  async function createPumpLaunchDraft(
    ...
    try {
      const preflightResponse = await window.mirae.preflightPumpLaunch({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: target.id,
  ...
  async function runLimitOrderSimulation(input: {
    ...
    try {
      const response = await window.mirae.simulateLimitOrder({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
        previewId: input.preview.id,
  ...
  async function runLimitOrderExecution(
    ...
    try {
      const response = await window.mirae.executeLimitOrder({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
        previewId: input.preview.id,
  ...
  async function runLimitCancelSimulation(input: {
    ...
    try {
      const response = await window.mirae.simulateLimitOrderCancel({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        walletAddress: input.walletAddress,
  ...
  async function runLimitCancelExecution(
    ...
    try {
      const response = await window.mirae.executeLimitOrderCancel({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
        walletAddress: input.walletAddress,
  ...
  async function verifyLimitOrderExecution(input: {
    ...
    try {
      const response = await window.mirae.verifyLimitOrderExecution({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
        previewId: input.preview.id,
  ...
  async function verifyLimitOrderCancel(input: {
    ...
    try {
      const response = await window.mirae.verifyLimitOrderCancel({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
  ...
  async function runExecution(
    ...
    try {
      const response = await window.mirae.executeMission({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
        missionId: input.preview.id,
  ...
  async function verifyExecution(input: {
    ...
    try {
      const response = await window.mirae.verifyMissionExecution({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        sessionId: input.sessionId,
        missionId: input.preview.id,
   ...
   <main className="workspace" data-theme="dark">
      ...
      <aside className="leftRail">
        <button
          className="railBrand"
          type="button"
          aria-label="Return to Mirae home"
     ...
     {modalOpen && (
      ...
      {showDriftPerpsPanel && active?.walletAddress && (
        <PerpsPanel
          walletAddress={active.walletAddress}
...
function AutomationSetupDcaCard({
  ...
  setup: import("@mirae/contracts").AutomationSetupDcaRequest;
...
function AutomationSetupExitCard({
  ...
  setup: import("@mirae/contracts").AutomationSetupExitRequest;
  onApprove: (payload: any) => Promise<void>;
}) {
  const [busy, setBusy] = useState(false);
  const [approved, setApproved] = useState(false);
