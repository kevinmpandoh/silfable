...
export function EmergencyStopPanel({
  ...
  async function engage(): Promise<void> {
    ...
    setMessage(null);
    try {
      const response = await window.mirae.engageEmergencyStop({
        schemaVersion: 1,
  ...
  async function release(): Promise<void> {
    ...
    setMessage(null);
    try {
      const response = await window.mirae.releaseEmergencyStop({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
  ...
  return (
    <div className="missionsView">
      <header className="missionsHeader">
        <div><p className="kicker">Mission current</p><h1>Review routes before execution.</h1><p>Inspect quote evidence, policy limits, and blocked checks. Opening a mission never executes it; simulation and approval remain separate.</p></div>
...
export function RightRail({
  ...
  const fetchAutomation = useCallback(async () => {
    try {
      if ((window as any).mirae?.listAutomationStrategies) {
        const res = await (window as any).mirae.listAutomationStrategies();
  ...
  const handleApproveProposal = async (proposalId: string) => {
    ...
    try {
      ...
      if ((window as any).mirae?.setAutomationStatus) {
        await (window as any).mirae.setAutomationStatus({
          schemaVersion: 1,
          requestId: crypto.randomUUID(),
          id: proposalId,
  ...
  useEffect(() => {
    ...
    const fetchPositions = async () => {
      try {
        const result = await window.mirae.getActivePositions();
     ...
     void (async () => {
      try {
        for (let attempt = 0; attempt < 10; attempt += 1) {
          const response = await window.mirae.getPortfolio({
            schemaVersion: 1,
            requestId: crypto.randomUUID(),
            ...
            <RailSection title="Position">
              ...
              <small>{pumpAsset ? `Token units at finalized slot ${portfolio?.slot.toLocaleString()}` : "No finalized balance for this mint was found in the selected wallet."}</small>
               ...
               <div className="pumpControlGrid" style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                {activePosition ? (
                  <button className="dangerOutline" onClick={() => window.mirae.closePosition(activePosition.id)}>
...
export function Composer({
  ...
  return (
    <div className={`composer ${disabled ? "disabled" : ""}`} onClick={() => textareaRef.current?.focus()}>
      <textarea
        ref={textareaRef}
        value={value}
        disabled={disabled}
