        ...
        {session.messages.map((message) => (
          <article className={message.role} key={message.id}>
            ...
            <div>
              ...
              {(message as any).perpAssessment && (
                <PerpAnalysisCard market={(message as any).perpMarket ?? "PERP"} assessment={(message as any).perpAssessment} />
              )}
              {(message as any).perpProposal && (
                <PerpProposalCard
                  ...
                  sessionId={session.id}
                  fullAccess={session.permission === "full" && !(message as any).perpManualApproval}
                />
              )}
              {(message as any).x402Resources && (
                <X402ResourcesCard message={message as any} restricted={session.permission === "restricted"} onExecute={(resource, masterPassword) => onExecuteX402(message.id, resource, masterPassword)} onAnalyze={(receiptIds) => onAnalyzeX402(message.id, receiptIds)} />
              )}
