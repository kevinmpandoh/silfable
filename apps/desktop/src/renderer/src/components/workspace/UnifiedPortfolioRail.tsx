...
export function UnifiedPortfolioRail({
  ...
  useEffect(() => {
    ...
    const tasks = solanaTargets.map((wallet) => async () => ({
      wallet,
      snapshot: (await window.mirae.getPortfolio({
        schemaVersion: 1,
  ...
  useEffect(() => {
    ...
    const tasks = requests.map(({ wallet, chain }) => async () => ({
      wallet,
      snapshot: (await window.mirae.getEvmPortfolio({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        chainKey: chain.key,
        address: wallet.address,
  ...
  useEffect(() => {
    ...
    if (typeof window.mirae.getPortfolioCostBasis !== "function") {
      setCostBasisSummary(null);
      return;
    }
    window.mirae.getPortfolioCostBasis({
      schemaVersion: 1,
      requestId: crypto.randomUUID(),
  ...
  useEffect(() => {
    ...
    window.mirae.listKaminoRwaPositions()
      .then((response) => {
        if (!active) return;
        setKaminoPositions(response.positions.filter((position) => solanaAddresses.has(position.walletAddress) && position.status === "CONFIRMED" && BigInt(position.amountSuppliedAtomic) > 0n));
      })
      .catch((err) => {
        if (!active) return;
