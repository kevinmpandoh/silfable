...
export function AutomationPanel({
  ...
  return (
    <div className="automationPanel space-y-6 p-6">
      <div className="automationHeader flex items-center justify-between flex-wrap gap-4">
        <div>
        ...
        <div className="flex items-center gap-3">
          <div className="automationFilters flex items-center gap-1 bg-slate-900/80 p-1 rounded-xl border border-slate-800 text-xs">
            <button
      ...
      <div className="automationHelp rounded-xl border border-emerald-500/20 bg-emerald-950/20 p-4 text-xs text-emerald-200/90 flex items-start gap-3">
      ...
      <div className="space-y-4">
        ...
        {displayStrategies.filter(s => s.status !== "CANCELLED").length === 0 ? (
          <Card className="automationEmpty p-8 text-center border-dashed border-slate-800 bg-slate-900/30">
            ...
            {displayStrategies.filter(s => s.status !== "CANCELLED").map((strategy) => {
              ...
              return (
                <Card
                  key={strategy.id}
                  variant="elevated"
                  className={`automationStrategyCard relative border-slate-800/80 bg-slate-900/60 transition-all ${strategy.sessionId && onSelectSession ? "cursor-pointer hover:border-emerald-500/50" : ""}`}
                  onClick={(e) => {
