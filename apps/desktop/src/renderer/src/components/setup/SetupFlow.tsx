...
export function WalletStep({
  ...
  async function refreshEvmWallets(): Promise<void> {
    const result = await window.mirae.getEvmWallets();
    setEvmAddress(result.address);
    setEvmWallets(result.wallets);
  }
  ...
  async function onboard(): Promise<void> {
    ...
    try {
      ...
      if (mode === "generate") {
        const result = await window.mirae.createWallet(base);
        ...
        const result = await window.mirae.importWalletMnemonic({
          ...base,
          mnemonic: secret,
        });
        setMessage(`Solana address ${shorten(result.address)} imported.`);
      } else {
        const result = await window.mirae.importWalletPrivateKey({
          ...base,
          privateKey: secret,
        });
        setMessage(`Solana address ${shorten(result.address)} imported.`);
      ...
      setSecret("");
      setRuntime(await window.mirae.getRuntimeStatus());
      setWallets((await window.mirae.listWallets()).wallets);
    } catch (error) {
      setSecret("");
      setMessage(
  ...
  async function createEvmWallet(): Promise<void> {
    ...
    setEvmRecovery(null);
    try {
      const result = await window.mirae.createRobinhoodWallet({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
  ...
  async function importEvmWallet(): Promise<void> {
    ...
    setEvmRecovery(null);
    try {
      await window.mirae.importRobinhoodWalletMnemonic({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
  ...
  async function importEvmPrivateKey(): Promise<void> {
    ...
    setEvmRecovery(null);
    try {
      await window.mirae.importRobinhoodWalletPrivateKey({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
  ...
  async function clearAllWallets(family: "solana" | "evm"): Promise<void> {
    ...
    try {
      if (family === "solana") {
        const result = await window.mirae.clearWallets({
          schemaVersion: 1,
          requestId: crypto.randomUUID(),
        ...
        const result = await window.mirae.clearEvmWallets({
          schemaVersion: 1,
          requestId: crypto.randomUUID(),
...
export function IntegrationStep({
  ...
  async function saveKey(): Promise<void> {
    ...
    setMessage(null);
    try {
      await window.mirae.saveJupiterKey({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        apiKey: jupiterKey,
  ...
  async function saveUniswapKey(): Promise<void> {
    ...
    setMessage(null);
    try {
      await window.mirae.saveUniswapKey({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        apiKey: uniswapKey,
        acknowledgedExternalQuoteProvider: true,
      });
      await window.mirae.testUniswapKey({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
      });
      setUniswapKey("");
  ...
  async function saveRobinhoodRpc(): Promise<void> {
    ...
    try {
      ...
      await window.mirae.testEvmRpc(request);
      await window.mirae.saveEvmRpcUrl(request);
      setRobinhoodRpcUrl("");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <small className="providerHint">
          Optional. If no custom endpoint is saved, Mirae uses its verified
...
export function TuningStep({
  ...
  async function saveAndContinue(): Promise<void> {
    ...
    setSaveMessage(null);
    try {
      await window.mirae.saveTransactionSettings({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        settings: {
      ...
      await window.mirae.savePumpRiskSettings({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
...
export function ProviderStep({
  ...
  async function loadModels(): Promise<void> {
    ...
    setMessage(null);
    try {
      const response = await window.mirae.previewOpenRouterModels({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        apiKey,
  ...
  async function saveProvider(): Promise<void> {
    ...
    setMessage(null);
    try {
      await window.mirae.saveAiProvider({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        provider: "openrouter",
        apiKey,
  ...
  return (
    <SetupCard
      ...
      subtitle="OpenRouter supplies the model; Mirae keeps authority and tool enforcement local."
    >
