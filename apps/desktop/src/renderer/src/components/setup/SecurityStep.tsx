...
import type { RuntimeStatus } from "@mirae/contracts";
...
export function SecurityStep({
  ...
  async function configure(): Promise<void> {
    ...
    setMessage(null);
    try {
      const response = await window.mirae.configureMasterPassword({
        schemaVersion: 1,
      ...
      onConfigured(
        runtime
          ...
          : await window.mirae.getRuntimeStatus(),
      );
    } catch (error) {
      ...
      if (
        ...
        setMessage(
          "The desktop runtime is outdated. Quit Mirae completely and reopen it before trying again.",
        );
      else if (/already configured/u.test(detail))
        setMessage(
          "A master password is already configured. Reopen Mirae and use the unlock screen.",
        );
        ...
        setMessage(
          "Password could not be saved. Restart Mirae and try again.",
        );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <SetupCard
      ...
      subtitle={
        migration
          ? "Create the master password that will be required whenever Mirae opens."
          : "Create the access password used to enter Mirae on this machine."
      }
    >
      {migration && (
...
export function ChangePasswordStep({ onContinue }: { onContinue: () => void }) {
  ...
  async function change(): Promise<void> {
    ...
    setMessage(null);
    try {
      await window.mirae.changeMasterPassword({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        currentPassword,
        newPassword,
  ...
  return (
    <SetupCard
      ...
      subtitle="Update the password required when Mirae opens."
    >
