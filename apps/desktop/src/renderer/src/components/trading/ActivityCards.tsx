...
export function EvmSwapProposalCard({
  ...
  async function openEvmExplorer(receipt: EvmSessionExecutionReceipt): Promise<void> {
    ...
    await window.mirae.openTransactionInExplorer({
      schemaVersion: 1,
        ...
        >
          ...
          {execution.depositSignature && (
            <div className="receiptActions">
              <button
                onClick={() =>
                  void window.mirae.copyTransactionSignature({
                    schemaVersion: 1,
                    requestId: crypto.randomUUID(),
              ...
              <button
                onClick={() =>
                  void window.mirae.openTransactionInExplorer({
                    schemaVersion: 1,
                    requestId: crypto.randomUUID(),
                    signature: execution.depositSignature!,
                  })
...
export function PumpLaunchDraftForm({
  ...
  const submit = async (): Promise<void> => {
    ...
    setError(null);
    try {
      if (typeof window.mirae.publishManagedPumpLaunchMetadata !== "function") {
        throw new Error("Pinata upload was added to the desktop preload. Fully quit and reopen Mirae before uploading a token image.");
      }
      const imageBase64 = await fileToBase64(imageFile);
      const published = await window.mirae.publishManagedPumpLaunchMetadata({
        schemaVersion: 1,
        requestId: crypto.randomUUID(),
        creatorWallet,
        name: name.trim(),
      ...
      setError(/No handler registered for ['"]pump:launch-managed-metadata-publish['"]/u.test(message)
        ? "The desktop main process is still using an older build. Quit Mirae from the tray or stop the desktop dev command, then start the app again. Reloading the window is not enough."
        : message);
    } finally {
      setSubmitting(false);
    }
...
export function ExecutionResult({
  ...
  async function copySignature(): Promise<void> {
    ...
    await window.mirae.copyTransactionSignature({
      schemaVersion: 1,
      requestId: crypto.randomUUID(),
