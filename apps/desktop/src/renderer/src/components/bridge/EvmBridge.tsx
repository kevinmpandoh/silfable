...
export function EvmBridgeWorkspace({
  ...
  useEffect(() => {
    let live = true;
    void window.mirae.listEvmBridgeReceipts().then((response) => {
      if (!live) return;
 ...
 function toRaw(value: string): string | null {
  ...
  async function prepare(): Promise<void> {
    ...
    setPrepared(null);
    try {
      const result = await window.mirae.prepareEvmBridge({
  ...
  async function reconcile(): Promise<void> {
    ...
    setError(null);
    try {
      const result = await window.mirae.reconcileEvmBridge({
    ...
    <section className="bridgeProposalCard">
      ...
      {receipt && <div className="bridgeReceiptPanel">
        ...
        <div className="receiptActions" style={{ marginTop: "8px" }}>
          <button onClick={() => void window.mirae.copyTransactionSignature({
            schemaVersion: 1,
            requestId: crypto.randomUUID(),
            signature: receipt.sourceSignature,
          })}>Copy signature</button>
          <button onClick={() => void window.mirae.openTransactionInExplorer({
            schemaVersion: 1,
            requestId: crypto.randomUUID(),
            signature: receipt.sourceSignature,
