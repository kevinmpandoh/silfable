...
export function UpdatePolicy() {
  return (
    <section className="border-t border-black/15 py-20 sm:py-28">
      <CurrentReveal>
      <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-[var(--sc-orange)]">Distribution policy</p>
        ...
        {channels.map((channel) => (
          <div key={channel.name} className="border-r border-b border-black/15 p-7">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="font-mono text-[8px] uppercase tracking-[0.14em] text-[var(--sc-violet)]">{channel.cadence}</span>
            </div>
            <p className="mt-12 text-sm leading-7 text-black/50">{channel.description}</p>
          </div>
        ))}
      </div>
      <div className="mt-8 border-l-2 border-[var(--sc-orange)] bg-[rgb(255_138_76_/_0.08)] p-5 text-sm leading-7 text-[var(--sc-pearl)]">
        Mirae does not resume a strategy automatically after an update. Review the release notes, reopen the app, and confirm which workflows should continue.
      </div>
      </CurrentReveal>
    </section>
  );
