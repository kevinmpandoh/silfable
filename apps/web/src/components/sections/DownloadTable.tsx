...
export async function DownloadTable() {
  ...
  let tag = "v0.1.0";
  try {
    const response = await fetch("https://api.github.com/repos/kevinmpandoh/silfable/releases/latest", { next: { revalidate: 3600 } });
  ...
  const releases: PlatformRelease[] = [
    ...
    { platform: "Linux", detail: "AppImage + Debian", requirement: "Preview artifacts and checksums available.", primaryLabel: "Download AppImage", href: `https://github.com/kevinmpandoh/silfable/releases/download/${tag}/Silfable-${version}-x86_64.AppImage` },
  ];
