...
export function EvmSwapPreviewCard({ proposal, onPrepare, busy }: { proposal: WebProposal; onPrepare: () => void; busy: boolean }) {
  ...
  return <SwapRouteCard
    ...
    statusLabel={confirmed ? "Confirmed" : hasQuote ? "Quote ready" : "Finding route"}
    ...
    details={[
      ...
      { label: "Route", value: "Uniswap · Robinhood" },
    ...
    helperText={hasQuote ? "Review the live route before the wallet opens. Token approval may be requested separately." : "Silfable is loading a live quote. Your wallet will not open automatically."}
    actionLabel={busy && !hasQuote ? "Loading Quote…" : busy ? "Preparing Review…" : "Review in Wallet"}
    actionDisabled={busy || final || !hasQuote}
