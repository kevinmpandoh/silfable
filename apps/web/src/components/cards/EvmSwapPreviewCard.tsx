...
function formatRaw(value: string | undefined, decimals: number): string {
  if (!value || !/^\d+$/u.test(value)) return "Not available";
...
export function EvmSwapPreviewCard({ proposal, onPrepare, busy }: { proposal: WebProposal; onPrepare: () => void; busy: boolean }) {
  ...
  return <SwapRouteCard
    ...
    details={[
      { label: "Input", value: proposal.sellAmount ? `${proposal.sellAmount} ${proposal.sellToken ?? ""}` : "Not available" },
      { label: "Expected output", value: hasQuote ? `${formatRaw(proposal.buyAmount, outputDecimals)} ${proposal.buyToken}` : "Loading quote…" },
      { label: "Minimum output", value: hasQuote ? `${formatRaw(proposal.minimumBuyAmount, outputDecimals)} ${proposal.buyToken}` : "Loading quote…" },
      { label: "Network fee", value: hasQuote ? networkFee : "Calculated at wallet review" },
    ...
    helperText={hasQuote ? "Review the live route before the wallet opens. Token approval may be requested separately." : busy ? "Mirae is loading a live quote. Your wallet will not open automatically." : "The previous quote attempt failed. Retry the route request; this does not open your wallet."}
