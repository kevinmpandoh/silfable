    ...
    <article className="min-w-0">
      <section id="introduction" className="scroll-mt-36 border-b border-black/15 pb-16">
        ...
        <p className="mt-9 max-w-2xl text-lg leading-8 text-black/55">
          Mirae brings Jupiter swaps, Pump.fun launches, Solana portfolio context, automation proposals, and wallet-reviewed execution into one operator workspace. Connected EVM routes remain available when a workflow needs to move beyond Solana.
        </p>
        <div className="mt-10 grid border-t border-l border-black/15 sm:grid-cols-3">
          {["Solana Mainnet", "Wallet reviewed", "Result tracked"].map((item) => (
            <div key={item} className="flex items-center gap-3 border-r border-b border-black/15 p-5 text-xs font-medium text-black">
              <Check className="size-4 text-[var(--sc-ice)]" /> {item}
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="quick-start" className="scroll-mt-36 border-b border-black/15 py-16">
        ...
        <p className="doc-lead">Choose web or desktop and connect the Solana wallet you intend to use. On web, network routes and provider credentials are managed by Mirae from the server environment.</p>
        <ol className="mt-10 space-y-7">
          {[
            ["Choose", "Use desktop for encrypted local-vault signing, or web with a connected browser wallet."],
            ["Enter workspace", "After wallet authentication, web users enter the dashboard immediately. Runtime limits, RPC routes, and provider credentials are managed automatically."],
            ["Describe", "Open a wallet-bound Solana workspace and state the swap, launch, portfolio, bridge, or automation outcome you want. Opening a workspace never authorizes a transaction."],
            ...
            <li key={title} className="grid gap-3 border-t border-black/10 pt-6 sm:grid-cols-[3rem_8rem_1fr]">
              <span className="font-mono text-sm text-[var(--sc-orange)]">{String(index + 1).padStart(2, "0")}</span>
      ...
      <section id="desk-rule" className="scroll-mt-36 border-b border-black/15 py-16">
        ...
        <p className="doc-lead">Global limits define the amounts, fees, slippage, and venues Mirae may present for confirmation. A new request cannot silently raise them.</p>
        <div className="mt-8 mb-8 overflow-hidden rounded-xl border border-black/10 shadow-sm">
          <Image src="/mirae-settings-orange.png" alt="Mirae desktop settings interface" width={1200} height={800} className="w-full object-cover" />
        </div>
        <div className="border-l-2 border-[var(--sc-orange)] bg-[rgb(255_138_76_/_0.08)] p-5 text-sm leading-7 text-[var(--sc-pearl)]">
          If required route data is missing, stale, or outside your configured limits, Mirae stops the action before wallet confirmation.
        </div>
      </section>
      ...
      <section id="mission-lifecycle" className="scroll-mt-36 border-b border-black/15 py-16">
        ...
        <div className="mt-10 space-y-0 border-t border-black/15">
          {[
            ["Prepared", "Mirae gathers the required parameters and creates a venue-specific transaction preview."],
      ...
      <section id="security" className="scroll-mt-36 border-b border-black/15 py-16">
        ...
        <p className="doc-lead">Mirae keeps wallet signing, route validation, provider responses, and transaction outcomes in clearly defined parts of the system.</p>
        <div className="mt-10 grid gap-5 sm:grid-cols-2">
          {[
            ...
            { icon: Monitor, title: "Checks before confirmation", text: "Mirae evaluates route and limit requirements before asking the active wallet to sign." },
        ...
        <div className="mt-9 overflow-x-auto">
          <table className="min-w-[48rem] w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-black/10 text-black/55">
              ...
              <tr><td className="py-5 text-ink">Bridge</td><td>Verified restricted routes from Solana to a connected EVM network and back</td><td>Provider route, liquidity, source confirmation, and destination settlement are checked per transfer</td></tr>
              <tr><td className="py-5 text-ink">Connected EVM Swap</td><td>Verified restricted execution on Robinhood Chain</td><td>A healthy RPC, live route, balance, gas, and explicit wallet approval are required</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section id="troubleshooting" className="scroll-mt-36 py-16">
        ...
        <div className="mt-10 divide-y divide-black/15 border-t border-black/15">
          {[
            ...
            ["A receipt is missing", "Check the local workspace path and available disk space. Mirae fails closed when receipt storage is unavailable."],
          ].map(([problem, answer]) => (
...
function DocHeading({ number, title }: { number: string; title: string }) {
  return (
    <div className="flex items-baseline gap-5">
      <span className="font-mono text-sm tracking-[0.18em] text-[var(--sc-orange)]">{number}</span>
