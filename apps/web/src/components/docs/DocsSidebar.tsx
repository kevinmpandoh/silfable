      ...
      <ScrollArea className="sticky top-20 z-30 -mx-5 w-[calc(100%+2.5rem)] border-b border-black/15 bg-paper px-5 lg:hidden">
        <nav aria-label="Documentation chapters" className="flex w-max gap-6 py-4">
          {chapters.map((chapter) => (
            <a key={`${chapter.href}-${chapter.label}`} href={chapter.href} className="text-[9px] font-semibold uppercase tracking-[0.16em] text-black/45 hover:text-electric">
              {chapter.label}
            </a>
          ))}
        </nav>
          ...
          <nav aria-label="Documentation chapters" className="border-l border-black/15 pr-4">
            {chapters.map((chapter, index) => (
              <a
                ...
                href={chapter.href}
                className="group flex items-center gap-4 border-l border-transparent py-2.5 pl-5 text-sm text-black/45 transition-colors hover:border-electric hover:text-ink"
              >
