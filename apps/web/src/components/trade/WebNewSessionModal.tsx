...
export function WebNewSessionModal({
  ...
  const [solanaLinkRequested, setSolanaLinkRequested] = useState(false);
  ...
  useEffect(() => {
    ...
    void connectSolanaWallet().catch((cause) => {
      setSolanaLinkRequested(false);
      setError(cause instanceof Error ? cause.message : "Solana wallet could not be connected.");
    });
  }, [connectSolanaWallet, isOpen, selectedSolanaWallet, solanaConnected, solanaConnecting, solanaLinkRequested]);

  useEffect(() => {
    if (!isOpen || !solanaLinkRequested || !solanaConnected || !solanaPublicKey) return;
    const verification = window.setTimeout(() => {
      setSolanaLinkRequested(false);
      void linkSolanaWallet();
    }, 0);
    return () => window.clearTimeout(verification);
  // linkSolanaWallet intentionally runs only after the requested adapter connects.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, solanaConnected, solanaLinkRequested, solanaPublicKey]);
            ...
            <div className="choiceGrid sessionWorkspaceChoices">
              {(["solana", "evm"] as const).map((value, index) => (
                <button type="button" key={value} className={workspace === value ? "active" : ""} onClick={() => setWorkspace(value)}>
                  ...
                  <small>{value === "solana" ? "Primary · Jupiter, Pump.fun, automation, and bridge" : "Connected EVM · supported Uniswap swaps and bridge"}</small>
                </button>
              ))}
            </div>
          </section>
            ...
            <div className="sessionWalletBinding">
              {workspace === "solana" ? (
                <>
                  ...
                  <button type="button" className="linkWalletButton" disabled={linking || solanaConnecting || solanaLinkRequested} onClick={() => void linkSolanaWallet()}>{linking ? "Awaiting wallet signature…" : solanaConnecting || solanaLinkRequested ? "Connecting Solana wallet…" : solanaConnected && solanaPublicKey?.toBase58() === effectiveSolanaAddress ? "Solana wallet connected" : effectiveSolanaAddress ? "Connect bound Solana wallet" : "+ Connect and verify Solana wallet"}</button>
                </>
              ) : (
                <>
