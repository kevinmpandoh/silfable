...
export async function POST(request: NextRequest) {
  try {
    const input = await request.json() as { walletAddress?: unknown; sellToken?: unknown; buyToken?: unknown; amount?: unknown; slippageBps?: unknown };
    ...
    if (!apiKey) return NextResponse.json({ error: "UNISWAP_API_KEY is not configured on the Mirae server." }, { status: 503 });
    ...
    const { response, payload: body } = await requestUniswapQuote(apiKey, { type: "EXACT_INPUT", amount: amountIn, tokenInChainId: CHAIN_ID, tokenOutChainId: CHAIN_ID, tokenIn, tokenOut, swapper: input.walletAddress, recipient: input.walletAddress, slippageTolerance: slippageBps / 100, routingPreference: "BEST_PRICE", protocols: ["V2", "V3", "V4"] });
    if (!response.ok) {
    ...
    if (!(routing === "CLASSIC" || canonicalWrap || canonicalUnwrap) || result?.permitData != null || quoteInput?.amount !== amountIn || String(quoteInput?.token).toLowerCase() !== tokenIn || String(quoteOutput?.token).toLowerCase() !== tokenOut || !RAW.test(String(quoteOutput?.amount ?? ""))) {
      return NextResponse.json({ error: "Uniswap returned a route that does not meet Mirae's Robinhood safety policy." }, { status: 502 });
    }
