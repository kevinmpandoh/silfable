...
export async function POST(request: NextRequest) {
  try {
    const input = await request.json() as { walletAddress?: unknown; quote?: unknown; routing?: unknown; tokenIn?: unknown; tokenOut?: unknown; amountIn?: unknown };
    ...
    if (tokenIn !== NATIVE_ETH && routing === "CLASSIC") {
      const approvalCheck = await uniswap("/check_approval", apiKey, { walletAddress: input.walletAddress, token: tokenIn, amount: amountIn, chainId: CHAIN_ID, urgency: "normal", includeGasInfo: true });
    ...
    const swap = await uniswap("/swap", apiKey, { quote, refreshGasPrice: true, simulateTransaction: true, safetyMode: "SAFE", urgency: "normal" });
