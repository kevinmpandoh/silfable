...
async function verifyConfirmedSignature(signature: string, walletAddress: string): Promise<void> {
  const response = await fetch(selectSolanaRpc(), {
    method: "POST",
...
export async function GET(request: NextRequest) {
  ...
  await evaluateOwnedStrategies(auth.userId, process.env.JUPITER_API_KEY?.trim() || undefined);
...
export async function PATCH(request: NextRequest) {
  ...
  if (action === "prepare") {
    ...
    const quote = await getQuote(proposal.inputMint, proposal.outputMint, proposal.amountRaw, Number(request.headers.get("x-slippage-bps") || 100), process.env.JUPITER_API_KEY?.trim() || undefined);
  ...
  if (action === "complete") {
    ...
    await verifyConfirmedSignature(parsed.data.transactionSignature, proposal.strategy.walletAddress);
