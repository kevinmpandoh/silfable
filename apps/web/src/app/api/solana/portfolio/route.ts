...
function tokenRows(payload: unknown): JupiterTokenRow[] {
  if (Array.isArray(payload))
    return payload.filter(
      (row): row is JupiterTokenRow => Boolean(row) && typeof row === "object"
    );
...
async function resolveTokenSymbols(
  ...
  for (let offset = 0; offset < uniqueMints.length; offset += 20) {
    ...
    const payload =
      tokenRows(primary).length > 0
        ? primary
        : await request(JUPITER_TOKENS_LITE_API);

    for (const row of tokenRows(payload)) {
      const mint = [row.id, row.address, row.mint].find(
        (value): value is string => typeof value === "string"
      );
      const symbol = typeof row.symbol === "string" ? row.symbol.trim() : "";
      if (mint && batch.includes(mint) && symbol)
        symbols.set(mint, symbol.slice(0, 24));
    }
  }
  ...
  if (body.error) {
    throw new Error(
      typeof body.error.message === "string"
        ? body.error.message
        : "The RPC rejected the balance request."
    );
  }
...
async function quoteAssetValueUsd(
  mint: string,
  rawAmount: string
): Promise<number> {
  ...
  const response = await fetch(url, {
    headers: process.env.JUPITER_API_KEY?.trim()
      ? { "x-api-key": process.env.JUPITER_API_KEY.trim() }
      : {},
    cache: "no-store",
    signal: AbortSignal.timeout(12_000),
  });
  const quote = (await response.json().catch(() => null)) as {
    outAmount?: unknown;
  } | null;
  return response.ok &&
    typeof quote?.outAmount === "string" &&
    /^\d+$/u.test(quote.outAmount)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const tokenBodies = await Promise.all(
      TOKEN_PROGRAMS.map(async (programId, index) => {
        const response = await fetch(DEFAULT_MAINNET_RPC, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            jsonrpc: "2.0",
            id: `mirae-portfolio-token-${index}`,
    ...
    assets.push({
      ...
      symbol: "SOL",
      amount: solResult.lamports / 1e9,
      rawAmount: String(solResult.lamports),
      valueUsd: 0,
    });
    ...
    >();
    for (const tokensBody of tokenBodies) {
      if (!Array.isArray(tokensBody?.result?.value)) continue;
      for (const item of tokensBody.result.value) {
        ...
        if (
          ...
          continue;
        const current = tokenBalances.get(mint);
        if (current && current.decimals !== decimals) continue;
        tokenBalances.set(mint, {
          rawAmount: (current?.rawAmount ?? 0n) + BigInt(rawAmount),
          decimals,
        });
      }
    }
    ...
    for (const [mint, balance] of tokenBalances) {
      ...
      assets.push({
        ...
        amount: Number(balance.rawAmount) / 10 ** balance.decimals,
        rawAmount: balance.rawAmount.toString(),
        valueUsd: 0,
      });
    }

    const tokenSymbols = await resolveTokenSymbols(
      assets
        .filter((asset) => asset.mint !== WRAPPED_SOL_MINT)
        .map((asset) => asset.mint)
    );
    for (const asset of assets) {
      asset.symbol =
        asset.mint === WRAPPED_SOL_MINT
          ? "SOL"
          : tokenSymbols.get(asset.mint) ?? shortMint(asset.mint);
    }

    const values = await Promise.all(
      assets.map((asset) =>
        quoteAssetValueUsd(asset.mint, asset.rawAmount).catch(() => 0)
      )
    );
    let totalUsd = 0;
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to load the Mainnet balance.";
