...
test("managed Pinata configuration rejects invalid credentials and gateways", () => {
  ...
  assert.throws(() => getManagedPinataConfig({ MIRAE_PINATA_JWT: "short" }), /invalid/u);
  assert.throws(
    () => getManagedPinataConfig({ MIRAE_PINATA_JWT: config.jwt, MIRAE_PINATA_GATEWAY: "http://gateway.example" }),
    /HTTPS/u,
  );
});
...
test("managed Pinata publishes public image then metadata and never constructs a chain transaction", async () => {
  ...
  const result = await service.publishTokenMetadata({
    ...
    websiteUrl: "https://mirae.example/token",
    xUrl: null,
    telegramUrl: null,
