...
const ORDER_VALIDITY_MS = 90_000;
const MIRAE_TRADER_PDA_INDEX = 0;
const MARKET_LIST_TTL_MS = 5 * 60_000;
const MARKET_FEED_TTL_MS = 20_000;
const MARKET_FEED_STALE_MS = 5 * 60_000;
const MARK_PRICE_TTL_MS = 15_000;
...
const cacheEntries = new Map<
  string,
  { expiresAt: number; staleUntil: number; value: Promise<unknown> }
>();
...
export async function getPerpCandles(
  candidate: string,
  timeframe = "1h",
  limit = 120
): Promise<PerpCandle[]> {
  ...
  if (!isAllowedSymbol(symbol))
    throw new Error("This perpetual market is not supported.");
  ...
  const candles = await cached(
    `candles:${symbol}:${timeframe}:${safeLimit}`,
    CANDLE_TTL_MS,
    () =>
      phoenixRequest<PerpCandle[]>(
...
async function buildPerpMarketFeed(): Promise<PerpMarketFeed> {
  ...
  const details = await mapWithConcurrency(selected, 1, async (market) => {
    const [mark, funding] = await Promise.all([
      cached(`mark:${market.symbol}`, MARK_PRICE_TTL_MS, () =>
        phoenixRequest<{
          slot: number;
          markPrice: { price: number; slot: number };
        }>(`/v1/market/${market.symbol}/mark-price`)
      ),
      ...
      cached(`funding:${market.symbol}`, FUNDING_TTL_MS, () =>
        ...
        }>(`/v1/funding/${market.symbol}/rates`)
      ).catch(() => ({
        rates: [] as Array<{
          timestamp: number;
  ...
  const markets = details.map(({ market, mark, funding }) => {
    ...
    return {
      ...
      openInterestBase:
        Number(market.statsSnapshot?.openInterestBaseLots ?? 0) * step,
  ...
  return {
    ...
    chainSlot,
    updatedAt: Date.now(),
    live: markets.some((market) => !market.stale),
  };
}

export async function getPerpAccount(
  walletAddress: string
): Promise<PerpAccountSnapshot> {
  const [walletUsdcBalance, marketFeed] = await Promise.all([
  ...
  try {
    state = await phoenixRequest<PhoenixTraderState>(
  ...
  const rawPositions = (state.snapshot?.subaccounts ?? []).flatMap(
    (subaccount) => subaccount.positions ?? []
  );
  const positions = rawPositions.flatMap((position) => {
    const symbol = normalizeSymbol(position.symbol);
    const market = marketFeed.markets.find(
      (candidate) => normalizeSymbol(candidate.symbol) === symbol
    );
    const baseLots = Number(position.basePositionLots ?? 0);
    const base = market ? baseLots * market.stepSizeBase : 0;
    if (!Number.isFinite(base) || base === 0) return [];
    const mark = market?.oraclePriceUsd ?? 0;
    return [
      {
        symbol: `${symbol}-PERP`,
        direction: base > 0 ? "long" : "short",
        baseAmount: Math.abs(base),
        entryPriceUsd: Number(position.entryPriceUsd ?? position.entryPrice ?? 0),
        markPriceUsd: mark,
        notionalUsd: Math.abs(base) * mark,
        unrealizedPnlUsd:
          base * (mark - Number(position.entryPriceUsd ?? position.entryPrice ?? 0)),
      } satisfies PerpPositionSnapshot,
    ];
  });
  const collateralUsd = (state.snapshot?.subaccounts ?? []).reduce(
    (sum, subaccount) =>
  ...
  return {
    ...
    leverage: collateralUsd > 0 ? totalNotionalUsd / collateralUsd : 0,
...
type PhoenixTraderState = {
  snapshot?: {
    subaccounts?: Array<{
      ...
      positions?: Array<{
        symbol: string;
        basePositionLots?: number | string;
...
export async function buildPerpOrderTransaction(
  ...
  if (!isAllowedSymbol(symbol))
    throw new Error(`${symbol}-PERP is not an allowlisted Mirae market.`);
  ...
  if (!market) throw new Error(`${symbol}-PERP is not currently active.`);
  if (market.stale) {
    throw new Error(
      `The ${market.symbol} mark price is ${market.oracleAgeSlots} slots behind, so it is not tradable right now.`
    );
  }

  const connection = new Connection(selectSolanaRpc(), "confirmed");
  const solBalanceLamports = await connection
  ...
  if (solBalanceLamports < 1_000_000) {
    const shortAddr =
      walletAddress.length > 10
        ...
        : walletAddress;
    throw new Error(
      `Your wallet (${shortAddr}) holds ${
        solBalanceLamports === 0 ? "0" : (solBalanceLamports / 1e9).toFixed(4)
      } SOL on Solana Mainnet. To cover on-chain network transaction fees, please fund at least 0.01 SOL to your wallet address.`
    );
  }
  ...
  if (!input.reduceOnly) {
    ...
    if (!account.accountExists) {
      ...
      if (solBalanceLamports < 40_000_000) {
        ...
        throw new Error(
          `Initializing a perpetuals trading account on Solana Mainnet requires an on-chain rent deposit (~0.0382 SOL / ~$2.95 USD). Your wallet holds ${solBal.toFixed(
            4
          )} SOL (~${needed} SOL short). Please fund a small additional amount of SOL (recommended: at least 0.05 SOL) to activate your on-chain trading account.`
        );
      }
    ...
    if (effectiveCollateral > 0) {
      ...
      if (Number.isNaN(available))
        throw new Error(
          "The wallet's USDC balance could not be verified, so no order was built."
        );
      if (available <= 0) {
        throw new Error(
          "This wallet holds 0 USDC. USDC collateral is required to open a position. Please swap SOL to USDC first."
        );
      }
      if (available < effectiveCollateral) {
        ...
        if (available >= minMargin) {
          ...
          throw new Error(
            `Insufficient USDC in wallet: order requires at least $${minMargin.toFixed(
              2
            )} USDC (at ${MAX_PERP_LEVERAGE}x max leverage), but wallet holds $${available.toFixed(
              2
            )} USDC.`
          );
        }
    ...
    if (notionalUsd > MAX_PERP_NOTIONAL_USD) {
      throw new Error(
        `Position notional $${notionalUsd.toFixed(
          2
        )} exceeds the guarded $${MAX_PERP_NOTIONAL_USD} per-order ceiling.`
      );
    }
    if (
      effectiveCollateral > 0 &&
      notionalUsd / effectiveCollateral > MAX_PERP_LEVERAGE
    ) {
      throw new Error(
        `This order implies ${(notionalUsd / effectiveCollateral).toFixed(
          1
        )}x leverage against $${effectiveCollateral.toFixed(
          2
        )} collateral, above the guarded ${MAX_PERP_LEVERAGE}x ceiling.`
      );
    }
  }
  ...
  const instructions = await phoenixRequest<ApiInstruction[]>(
    "/v1/ix/place-isolated-market-order",
    {
      ...
      body: JSON.stringify({
        ...
        ...(effectiveCollateral > 0
          ? { transferAmount: usdcToBaseUnits(String(effectiveCollateral)) }
          : {}),
      }),
    }
  );
  ...
  if (simulation.value.err)
    throw new Error(
      describeSimulationError(simulation.value.err, simulation.value.logs)
    );
  if (fee.value === null)
    throw new Error("The order network fee could not be verified.");
  ...
  if (blockHeight > blockhash.lastValidBlockHeight)
    throw new Error("The order blockhash expired during preflight.");
  ...
  return {
    ...
    checks: [
      ...
      input.reduceOnly
        ...
        : `Notional $${notionalUsd.toFixed(
            2
          )} is within the guarded $${MAX_PERP_NOTIONAL_USD} ceiling.`,
      "Only the verified exchange program and Solana system programs were invoked.",
...
export async function buildRegisterTraderTransaction(
  walletAddress: string
): Promise<PerpOrderPlan> {
  ...
  if (!ix || !ix.programId)
    throw new Error(
      "The perpetuals service returned no account-registration instruction."
    );
  ...
  if (fee.value === null)
    throw new Error("The network fee could not be verified.");
  const programs = invokedPrograms(simulation.value.logs);
  ...
  return {
    ...
    expiresAt: Date.now() + ORDER_VALIDITY_MS,
    checks: [
      "Initializes your perpetuals trading account on Solana Mainnet.",
      "Unsigned Mainnet simulation completed successfully.",
      "Your browser wallet remains the only signer.",
    ],
  };
...
function resolveQuantity(
  ...
  if (!Number.isFinite(raw) || raw <= 0)
    throw new Error(
      "Provide either a base size or a USD notional for the position."
    );
  ...
  const rounded = Number(
    stepped.toFixed(Math.max(0, Math.round(-Math.log10(market.stepSizeBase))))
  );
  if (rounded < market.minOrderBase) {
    const minUsd = (market.minOrderBase * market.oraclePriceUsd).toFixed(2);
    throw new Error(
      `The minimum ${market.symbol} order size is ${market.minOrderBase} ${
        market.baseAssetSymbol
      } (~$${minUsd} USD). Order size of $${
        input.notionalUsd ?? raw
      } is below the contract lot minimum.`
    );
  }
  return rounded;
}
...
export function invokedPrograms(logs: string[] | null | undefined): string[] {
  ...
  if (logs.some((line) => /log truncated/iu.test(line)))
    throw new Error("Simulation logs were truncated.");
  return [
    ...new Set(
      logs
        .map(
 ...
 */
export function assertAllowedPrograms(
  programs: string[],
  ...
  const exchangePrograms = instructions.map(
    (instruction) => instruction.programId
  );
  if (!exchangePrograms.some((program) => programs.includes(program))) {
    throw new Error("Simulation did not invoke the verified exchange program.");
  }
  const denied = programs.find((program) => !allowed.has(program));
  if (denied)
    throw new Error(`Simulation invoked a non-allowlisted program: ${denied}`);
}
...
export function describeSimulationError(
  ...
  const anchorError = logs
    ?.find((line) => /Error Message:/u.test(line))
    ?.split("Error Message:")[1]
    ?.trim();
  if (anchorError)
    return `The perpetuals venue rejected the simulated order: ${anchorError}`;
  if (logs?.some((line) => /insufficient/iu.test(line))) {
    return "The order was rejected for insufficient collateral. Deposit more USDC or reduce the size.";
  }
