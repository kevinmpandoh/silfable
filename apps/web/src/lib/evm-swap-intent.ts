...
export type SwapWorkspace = "solana" | "evm";
...
const DESTINATION_ADDRESS = new RegExp(`(?:ke|to|->|→)\\s*${TOKEN}\\s*(?:address|contract|alamat)(?:nya)?\\s*(?::|=|adalah|is|ke)?\\s*(0x[0-9a-fA-F]{40})`, "iu");
...
export function resolveRobinhoodSwapIntent(message: string): RobinhoodSwapIntent {
  ...
  const explicitDestination = message.match(DESTINATION_ADDRESS)?.[2];
  const buyToken = explicitDestination ?? normalize(pair[offset + 1]);
...
export function shouldHandleRobinhoodSwap(workspace: SwapWorkspace, intent: RobinhoodSwapIntent): boolean {
  return workspace === "evm" && intent.requested;
}
