...
const OTHER_INTENT = /\bdca\b|\bautomation\b|\botomasi\b|\btake[ -]?profit\b|\bstop[ -]?loss\b|\b(?:tp|sl)\b|\bswap\b|\btukar\b|\bconvert\b|\blaunch\b|\bbeli\b|\bjual\b|\bbuy\b|\bsell\b/iu;
...
export function resolveSolanaBridgeIntent(messages: BridgeConversationMessage[]): SolanaBridgeIntent {
  ...
  if (bridgeAnchor !== lastUserIndex) {
    if (OTHER_INTENT.test(lastUserContent) || !BRIDGE_FOLLOW_UP.test(lastUserContent)) {
