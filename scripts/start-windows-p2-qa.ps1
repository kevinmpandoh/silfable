param(
  [string]$Executable = "apps/desktop/release/win-unpacked/mirae.exe",
...
$installer = Get-ChildItem -LiteralPath (Join-Path $repoRoot "apps/desktop/release") `
  -Filter "Mirae-*-windows-x64-setup.exe" -File -ErrorAction SilentlyContinue |
...
$manifest = [ordered]@{
  ...
  checklist = "docs/MIRAE_PROJECT_REFERENCE.md"
}
...
Follow the Windows P2 acceptance checklist in docs/MIRAE_PROJECT_REFERENCE.md.
...
Write-Host "Mirae P2 QA launched with an isolated profile."
Write-Host "Evidence: $evidencePath"
Write-Host "Checklist: docs/MIRAE_PROJECT_REFERENCE.md"
