...
const installers = entries.filter((name) => /^Mirae-[0-9]+\.[0-9]+\.[0-9]+-windows-x64-setup\.exe$/u.test(name));
if (!unpackedOnly) {
...
const executablePath = join(unpackedDirectory, "mirae.exe");
...
for (const name of packagedFiles) {
  const firstParty = !name.startsWith("node_modules/") || name.startsWith("node_modules/@mirae/");
