...
while (Date.now() < rendererDeadline) {
  const rendererEvaluation = await send("Runtime.evaluate", {
    expression: "JSON.stringify({ bridgeAvailable: typeof window.mirae === 'object', rootHasContent: (document.querySelector('#root')?.textContent?.trim().length ?? 0) > 0, startupFailed: document.querySelector('.startupFailure') !== null, readyState: document.readyState })",
    returnByValue: true,
  });
