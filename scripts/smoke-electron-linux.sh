...
if [[ "$mode" == "wayland" ]]; then
  ...
  weston --backend=headless-backend.so --socket=wayland-mirae-qa --idle-time=0 >"${log}.weston" 2>&1 &
  weston_pid=$!
  for _ in $(seq 1 30); do
    [[ -S "$runtime_dir/wayland-mirae-qa" ]] && break
    kill -0 "$weston_pid" 2>/dev/null || { cat "${log}.weston"; exit 1; }
    sleep 0.25
  done
  [[ -S "$runtime_dir/wayland-mirae-qa" ]]
  WAYLAND_DISPLAY=wayland-mirae-qa XDG_SESSION_TYPE=wayland XDG_CONFIG_HOME="$config_dir" \
