...
for (const name of [...appImages, ...debs]) {
  assert.match(name, /^Mirae-[0-9]+\.[0-9]+\.[0-9]+-[A-Za-z0-9_]+\.(?:AppImage|deb)$/u);
...
const executable = join(unpackedDirectory, "mirae");
...
for (const name of packagedFiles) {
  ...
  const isFirstParty = !name.startsWith("node_modules/") || name.startsWith("node_modules/@mirae/");
  if (isFirstParty) {
    assert.equal(
...
assert.equal(
  ...
  false,
  "Packaged Mirae runtime must not contain source maps",
);
